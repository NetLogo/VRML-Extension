...
to recolor-patch  ;; patch procedure
  ...
  [ ifelse food > 0
    ...
    [ set pcolor scale-color green chemical 10 0.1 ] ]
end
...
to-report pzcor
  let z food * 0.25 + (0.3 * (ln (chemical + 0.1)) / max [ln (chemical + 0.1) ] of patches)
  if (nest?) 
    [ set z z - 0.3 ]
  report z
end
...
to export-vrml
  vrml:clear-scene
  vrml:set-color 255 255 255
;  vrml:add-box 0 0 -0.02 world-width world-height 1
   vrml:add-box world-width / 4 + 3 0 -0.02 world-width / 2 - 6 world-height 1
   vrml:add-box 0 - world-width / 4 - 3 0 -0.02 world-width / 2 - 6 world-height 1
   vrml:add-box 0 world-width / 4 + 3 -0.02 world-width world-height / 2 - 6 1
   vrml:add-box 0 0 - world-width / 4 - 3 -0.02 world-width world-height / 2 - 6 1
 
  ask patches [
    if (pcolor mod 10 < 9.9)
    [
      let rgbcol pcolor
      if (not is-list? rgbcol)
        [ set rgbcol extract-rgb pcolor ]
      vrml:set-color (item 0 rgbcol) (item 1 rgbcol) (item 2 rgbcol)
      vrml:add-box pxcor pycor pzcor 1 1 1
    ]
  ]
  ask turtles [
    let rgbcol color
    ...
    let zcor pzcor + 0.8
    vrml:set-color (item 0 rgbcol) (item 1 rgbcol) (item 2 rgbcol)
    vrml:add-sphere xcor - dx / 2 ycor - dy / 2 zcor .5
    vrml:add-sphere xcor ycor zcor 0.4
    vrml:add-sphere xcor + dx / 2  ycor + dy / 2 zcor 0.4

    vrml:add-cylinder xcor + dx / 2  ycor + dy / 2 zcor
                      (xcor + dx / 2) + dx / 3 + dy / 2  ycor + dy / 2 + dy / 3 - dx / 2 zcor + 0.3
                      0.1
    vrml:add-cylinder xcor + dx / 2  ycor + dy / 2 zcor
                      (xcor + dx / 2) + dx / 3 - dy / 2  ycor + dy / 2 + dy / 3 + dx / 2 zcor + 0.3
                      0.1
